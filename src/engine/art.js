...
export class Art {
  ...
  drawAsset(ctx,id,x,y,opts={}){
    ...
    const scale=opts.scale ?? 1.1, alpha=opts.alpha ?? 1;
    let w, h;
  ...
  drawTileAsset(ctx,id,x,y){ const img=this.sprites[id]; if(!img) return false; const dx=Math.floor(x), dy=Math.floor(y); ctx.drawImage(img,dx,dy,TILE,TILE); return true; }
  ...
  drawPlayer(ctx,x,y,dir='down',walk=0,charge=0,tool='hand',facing='right',attackCd=0){
    if(this.assetsReady){
      ...
      const flip = baseDir==='side' && facing==='left';
      this.shadow(ctx,x,y+10,18,6,.24);
  ...
  drawMonkey(ctx,x,y,walk=0,order=null,selected=false,entity=null){
    if(this.assetsReady){
      const face = entity?.facing || 'right';
      let id = entity?.carry ? 'monkey_carry' : (order?.type==='combat' ? 'monkey_attack' : `monkey_walk_${Math.floor(walk*7)%3}`);
      if(!entity?.carry && face==='down') id='monkey_down';
      if(!entity?.carry && face==='up') id='monkey_up';
      const flip = face==='left';
      this.shadow(ctx,x,y+8,14,5,.24);
